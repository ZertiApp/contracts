...
contract Vote is Initializable {
     ...
     *
     * Reward System: Generates a pool of ether for later distribution between winners(majority)
     *
     *  @param _userVote users vote obtained in front-end. 1 - In favor vote; 0 - Opposing vote
     */
    function receiveVote(uint8 _userVote) external payable CanVote {
     ...
     */
    function distributePool(uint8 _result, uint256 _amount) internal CanVote {
        require(
            block.timestamp >= startTime && block.timestamp <= endTime,
            "Voting has ended"
        );
        for (uint256 i = 0; i < voters[_result].length; ) {
